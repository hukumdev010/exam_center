...
from models import UserProgress as UserProgressModel, Certification as CertificationModel, Category as CategoryModel
...
async def get_user_progress(
    ...
    try:
        stmt = select(UserProgressModel).options(
            selectinload(UserProgressModel.certification).selectinload(CertificationModel.category)
        ).where(UserProgressModel.user_id == current_user.user.id).order_by(UserProgressModel.last_active_at.desc())
...
async def update_user_progress(
    ...
    try:
        ...
        cert_stmt = select(CertificationModel).where(CertificationModel.id == progress_data.certification_id)
